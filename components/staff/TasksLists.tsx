...
import { Flag, LayoutList } from 'lucide-react'
import { Progress } from '../ui/progress'
import { formatDateTiny, multiFormatDateString } from '@/lib/utils'
import { useRouter } from 'next/navigation'
import { useGetAllStaffTasks } from '@/query/client/taskQueries'

const TasksLists = ({ currentUser, tasktype }: { currentUser: any, tasktype: TaskTypes }) => {
    const router = useRouter();
    const [ tasklist, setTaskList ] = useState<any[]>([])
    const { data: allTasks, isLoading: loadingTasks } = useGetAllStaffTasks(currentUser?._id);

    useEffect(() => {
        if(allTasks){
            const filteredTasks = allTasks?.filter((task: any) => task?.Status == tasktype);
