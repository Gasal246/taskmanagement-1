...
import { useAddAreaStaff, useAddDepStaff, useGetAvailableStaffs } from '@/query/client/depQueries'
...
const AddAreaStaff = ({ trigger, depId, regId, areaId }: { trigger: React.ReactNode, depId: string, regId: string, areaId: string }) => {
    const { data: session }: any = useSession();
    const { data: stafflist, isLoading: loadingStafflist } = useGetAvailableStaffs(session?.user?.id, ['staff']);
    const { mutateAsync: addDepStaff, isPending: addingDepStaff } = useAddAreaStaff();
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        const response = await addDepStaff({ depid: depId, regid: regId, areaid: areaId, staffid: values.staffid });
    ...
    return (
        <Sheet>
            ...
            <SheetContent className='min-w-full lg:min-w-[600px] border-slate-700'>
                <SheetHeader>
                    <SheetTitle>Add Area Staffs</SheetTitle>
                ...
                <div>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="w-2/3 space-y-6">
                            ...
                            />
                            <Button type="submit">{addingDepStaff ? 'Adding...': 'Add Staff'}</Button>
                        </form>
                    </Form>
                </div>
            </SheetContent>
