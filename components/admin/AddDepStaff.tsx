...
import { useAddDepStaff, useGetAvailableStaffs } from '@/query/client/depQueries'
...
const AddDepStaff = ({ trigger, depId }: { trigger: React.ReactNode, depId: string }) => {
    const { data: session }: any = useSession();
    const { data: stafflist, isLoading: loadingStafflist } = useGetAvailableStaffs(session?.user?.id, ['staff', 'reg-staff', 'dep-staff']);
    const { mutateAsync: addDepStaff, isPending: addingDepStaff } = useAddDepStaff();
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        const response = await addDepStaff({ depid: depId, staffid: values.staffid });
    ...
    return (
        <Sheet>
            ...
            <SheetContent className='min-w-full lg:min-w-[600px] border-slate-700'>
                <SheetHeader>
                    <SheetTitle>Add Department Staffs</SheetTitle>
