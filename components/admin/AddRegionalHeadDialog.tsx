...
import { useAddRegionHead, useGetSelectableRegionalHeads, useGetStaffsRegionArea } from '@/query/client/adminQueries'
...
const AddRegionalHeadDialog = ({ trigger, regionId }: { trigger: React.ReactNode, regionId: string }) => {
    ...
    const { data: regionalHeads, isLoading: loadingReginalHead } = useGetSelectableRegionalHeads(session?.user?.id);
