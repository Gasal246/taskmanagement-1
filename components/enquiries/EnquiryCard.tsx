...
import { EMPTY_ENQUIRY_CATALOGUE, getCatalogueClassificationLabels } from "@/lib/enquiries/catalogue";
...
export default function EnquiryCard({ enquiry: e, number, basePath, staff = false }: { enquiry: any; number: number; basePath: string; staff?: boolean }) {
  ...
  const catalogue = catalogueData?.catalogue || EMPTY_ENQUIRY_CATALOGUE;
  const facility = e.camp_id || {};
  const classification = getCatalogueClassificationLabels(catalogue, facility);
  const projectSector = facility.project_sector
    ? isCatalogueLoading ? "Loading…" : classification.sector || facility.project_sector
    : "Not classified";
  const facilityType = facility.facility_type
    ? isCatalogueLoading ? "Loading…" : classification.facility || facility.facility_type
    : facility.camp_type || "Not classified";
  return <article className={`group relative rounded-xl border bg-gradient-to-br from-slate-900/90 to-slate-950 p-4 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-lg focus-within:ring-2 focus-within:ring-cyan-500 ${complete ? "border-emerald-900/70 hover:border-emerald-600/70" : "border-slate-700 hover:border-cyan-700"}`}>
    <div className="flex flex-wrap items-start justify-between gap-3">
      <Link href={`${basePath}/${e._id}`} className="min-w-0 flex-1 rounded-md outline-none after:absolute after:inset-0 after:rounded-xl focus-visible:ring-2 focus-visible:ring-cyan-400">
        ...
        <h2 className="mt-2 break-words text-base font-semibold text-slate-100 group-hover:text-cyan-100">{e.camp_id?.camp_name || "Facility not linked"} <ArrowUpRight size={16} className="inline text-slate-500" /></h2>
      </Link>
    ...
    <dl className="my-4 grid grid-cols-2 gap-x-5 gap-y-3 text-xs sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
      {[["Status", e.status || "Not set"], ["Priority", staff ? e.forwarded_priority ?? e.priority : e.priority], ["Project Sector", projectSector], ["Facility Type", facilityType], ["Occupancy", e.camp_id?.camp_occupancy ?? "Not specified"], ["WiFi", e.wifi_available === true ? "Yes" : e.wifi_available === false ? "No" : "Not specified"], ["Due date", enquiryDate(e.due_date)]].map(([label, value]) => <div key={label}><dt className="mb-1 text-slate-500">{label}</dt><dd className="break-words font-medium text-slate-200">{value || "Not set"}</dd></div>)}
    </dl>
