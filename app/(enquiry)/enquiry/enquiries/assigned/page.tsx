...
import { useGetEqAreas, useGetEqCampsByArea, useGetEqCities, useGetEqCountries, useGetEqProvince, useGetEqRegions, useGetUserAssignedEnquiries } from "@/query/enquirymanager/queries";
...
export default function EnquiriesPage() {
  ...
  // Queries
  const { data: enquiries, isLoading } = useGetUserAssignedEnquiries(filters);
    ...
    <div className="p-5 pb-10">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Assignet to you</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      {/* HEADER */}
      <div className="bg-gradient-to-tr from-slate-950/50 to-slate-900/50 p-3 rounded-lg mb-4 flex justify-between items-center">
        <h1 className="font-semibold text-sm text-slate-300 flex items-center gap-1">
          <PanelsTopLeft size={16} /> Enquiries assigned to you
        </h1>
      </div>
        ...
        <div className="space-y-2">
          {enquiries?.enquiries?.map((e: any) => (
            <div
              ...
              onClick={() => router.replace(`/enquiry/enquiries/${e._id}?from=assigned`)}
            >
