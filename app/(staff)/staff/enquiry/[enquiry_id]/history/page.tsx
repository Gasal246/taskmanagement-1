          ...
          {forwardHistories.map((h: any) => (
            ...
            >
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-lg font-semibold">Step {h.history_id?.step_number}</h3>
              ...
              <div className="text-sm space-y-2">
                ...
                <p className="flex items-center gap-2 text-slate-300">
                  ...
                  <b>{renderActor(h.history_id?.changed_by || h.history_id?.forwarded_by)}</b>
                </p>
          ...
          {updateHistories.map((h: any) => (
            ...
            >
              <div className="flex items-center gap-3 rounded-md border border-slate-700 bg-slate-950/40 p-3">
                <Avatar src={getActor(h.history_id)?.avatar_url || undefined} size={36}>
                  {getActorInitial(getActor(h.history_id))}
                </Avatar>
                <div className="min-w-0">
                  <p className="text-xs text-slate-400">Updated By</p>
                  <p className="truncate text-sm font-semibold text-slate-200">
                    {renderActor(getActor(h.history_id))}
                  </p>
                  <p className="flex items-center gap-1 truncate text-xs text-slate-400">
                    <Mail size={12} />
                    {getActor(h.history_id)?.email || "N/A"}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-md border border-slate-700 bg-slate-950/40 p-3 space-y-1">
                ...
                {Array.isArray(h.history_id?.changed_fields) && h.history_id.changed_fields.length > 0 ? (
                  h.history_id.changed_fields.map((change: any, index: number) => (
