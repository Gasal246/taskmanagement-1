...
import { useAddNewProject, useGetBusinessClients, useGetBusinessRegions } from '@/query/business/queries';
...
const AddNewProject = () => {
  ...
  const fetchCookieData = () => {
    ...
    if(!cookieData || !userCookie) return toast.error("No Cookie data available");
    const jsonData = JSON.parse(cookieData);
    const roleData = JSON.parse(userCookie);
    console.log("business_id: ", jsonData?.business_id);
    
    setBusinessId(jsonData?.business_id);
    handleFetchBusinessClients(jsonData?.business_id);
    setUserData({
      role_id: roleData?._id,
      dept_id: jsonData?.department_id
    })
  }
  ...
  const handleFetchBusinessClients = async (businessId: string) => {
    const res = await getBusinessClients(businessId);
  ...
  useEffect(()=>{
    ...
    form.setValue("role_id", userData.role_id);
    form.setValue("dept_id", userData.dept_id);
  }, [businessId, userData, form]);
  ...
  return (
    <div className="p-4">
      <Breadcrumb className='mb-3'>
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink onClick={() => router.back()}>Projects</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
