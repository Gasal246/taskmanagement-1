        ...
        <div className="flex flex-col gap-3 rounded-xl border border-slate-800/80 bg-gradient-to-r from-cyan-950/35 via-slate-900/75 to-emerald-950/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
              <MapPinned size={18} className="text-cyan-300" /> Regions Management
            </h1>
            <p className="text-xs text-slate-400">
              Organize regions by country with clarity and control for enquiries.
            </p>
          </div>
          <Link href="/admin/enquiries/regions/add-region">
            <Button className="w-full sm:w-auto bg-cyan-700 hover:bg-cyan-600 text-white rounded-e-full rounded-s-lg">
              <Plus size={14} className="mr-1" /> Add New Region
            </Button>
          </Link>
        </div>
        ...
        <div className="rounded-xl border border-slate-800/80 bg-gradient-to-b from-slate-900/70 to-slate-950/70 p-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 mb-3">
            ...
            <Select value={country_id} onValueChange={setCountry}>
              <SelectTrigger className="bg-slate-900/50 text-slate-200 border-slate-700">
            ...
            <table className="min-w-full text-sm text-slate-300">
              <thead className="bg-slate-900/90">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-300">Region Name</th>
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={5} className="p-6 text-center text-slate-400">Loading regions...</td></tr>
                ) : regionList.length === 0 ? (
                  <tr><td colSpan={5} className="p-6 text-center text-slate-400">No regions found.</td></tr>
                ) : (
                  regionList.map((region: any, index: number) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300">{region.country_id?.country_name}</td>
                      <td className="px-4 py-3 font-medium text-slate-100">{region.region_name}</td>
                      <td className="px-4 py-3 text-slate-300">{region.updatedAt ? new Date(region.updatedAt).toLocaleDateString() : "-"}</td>
