...
import { useGetEqHeadOfficeProfile, useRemoveEqHeadOffice } from "@/query/enquirymanager/queries";
...
export default function HeadOfficeDetailsPage() {
  ...
  const { data: headOfficeData } = useGetEqHeadOfficeProfile(params.head_office_id);
  const { mutateAsync: RemoveHeadOffice, isPending: isDeleting } = useRemoveEqHeadOffice();
  ...
  const handleDeleteHeadOffice = async () => {
    ...
    if (res?.status === 200) {
      ...
      return router.replace("/admin/enquiries/camps/head-offices");
    }
      ...
      <div className="bg-gradient-to-tr from-slate-900 to-slate-800 p-5 rounded-xl mb-6 border border-slate-700 flex justify-between items-start">
        ...
        <div className="flex flex-col gap-2">
          <Button
            ...
            onClick={() => router.push(`/admin/enquiries/camps/head-offices/${params.head_office_id}/edit_head_office`)}
          >
            <Pencil size={14} /> Edit
          </Button>
          <Button
        ...
        {camps.length === 0 ? (
          ...
          <div className="divide-y divide-slate-800">
            {camps.map((camp: any) => (
              <div
                key={camp._id}
                className="py-3 flex items-center justify-between cursor-pointer hover:text-cyan-200"
                onClick={() => router.push(`/admin/enquiries/camps/${camp._id}`)}
              >
                ...
                <span className="text-xs text-slate-500">View Camp</span>
              </div>
            ))}
          </div>
        )}
