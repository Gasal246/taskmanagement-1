...
import { useGetEqHeadOfficeProfile, useUpdateEqHeadOffice, useGetEqCampsFiltered } from "@/query/enquirymanager/queries";
import {
    Pagination,
    PaginationContent,
    PaginationEllipsis,
...
export default function EditHeadOfficePage() {
    ...
    const { data: headOfficeData, isLoading } = useGetEqHeadOfficeProfile(params.head_office_id);
    const { mutateAsync: UpdateHeadOffice, isPending: isUpdating } = useUpdateEqHeadOffice();
    const { data: campsData, isLoading: isCampsLoading } = useGetEqCampsFiltered({ search: campSearch, page: campPage, limit: campLimit });
    ...
    const onSubmit = async (values: any) => {
        ...
        if (res?.status == 200) {
            ...
            return router.replace(`/admin/enquiries/camps/head-offices/${params.head_office_id}`);
        }
        ...
        <div className="p-5 pb-10">
            <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink onClick={() => router.replace("/admin/enquiries/camps/head-offices")}>Head Offices</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
