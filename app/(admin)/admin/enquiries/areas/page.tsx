      ...
      <div className="w-full p-1 space-y-4">
        <div className="flex flex-col gap-3 rounded-xl border border-slate-800/80 bg-gradient-to-r from-cyan-950/35 via-slate-900/75 to-emerald-950/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
              <LandPlot size={18} className="text-cyan-300" /> Areas Management
            </h1>
            <p className="text-xs text-slate-400">
              Manage areas with full location hierarchy for consistent enquiries.
            </p>
          </div>
          <Link href="/admin/enquiries/areas/add-area">
            <Button className="w-full sm:w-auto bg-cyan-700 hover:bg-cyan-600 text-white rounded-e-full rounded-s-lg">
              <Plus size={14} className="mr-1" /> Add New Area
            </Button>
          </Link>
        </div>
        ...
        <div className="rounded-xl border border-slate-800/80 bg-gradient-to-b from-slate-900/70 to-slate-950/70 p-3">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-slate-200">Areas</h2>
            ...
            <table className="min-w-full text-sm text-slate-300">
              <thead className="bg-slate-900/90">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-300">Area Name</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-300">Action</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={7} className="p-6 text-center text-slate-400">Loading areas...</td></tr>
                ) : areaList.length === 0 ? (
                  <tr><td colSpan={7} className="p-6 text-center text-slate-400">No areas found.</td></tr>
                ) : (
                  areaList.map((area: any, index: number) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300">{area.country_id?.country_name}</td>
                      <td className="px-4 py-3 text-slate-300">{area.region_id?.region_name}</td>
                      <td className="px-4 py-3 text-slate-300">{area.province_id?.province_name}</td>
                      <td className="px-4 py-3 text-slate-300">{area.city_id?.city_name}</td>
                      <td className="px-4 py-3 font-medium text-slate-100">{area.area_name}</td>
                      <td className="px-4 py-3">
                        <div className="flex justify-end">
                          <Button
                            ...
                            onClick={() => router.push(`/admin/enquiries/areas/${area?._id}`)}
                          >
