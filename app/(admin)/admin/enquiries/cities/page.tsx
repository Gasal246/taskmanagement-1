      ...
      <div className="w-full p-1 space-y-4">
        <div className="flex flex-col gap-3 rounded-xl border border-slate-800/80 bg-gradient-to-r from-cyan-950/35 via-slate-900/75 to-emerald-950/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <h1 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
              <Building2 size={18} className="text-cyan-300" /> Cities Management
            </h1>
            <p className="text-xs text-slate-400">
              Manage city records with clear province and region hierarchy.
            </p>
          </div>
          <Link href="/admin/enquiries/cities/add-city">
            <Button className="w-full sm:w-auto bg-cyan-700 hover:bg-cyan-600 text-white rounded-e-full rounded-s-lg">
              <Plus size={14} className="mr-1" /> Add New City
            </Button>
          </Link>
        </div>
        ...
        <div className="rounded-xl border border-slate-800/80 bg-gradient-to-b from-slate-900/70 to-slate-950/70 p-3">
          <div className="mb-3 flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-slate-200">Cities</h2>
            ...
            <table className="min-w-full text-sm text-slate-300">
              <thead className="bg-slate-900/90">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-300">City Name</th>
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={7} className="p-6 text-center text-slate-400">Loading cities...</td></tr>
                ) : cityList.length === 0 ? (
                  <tr><td colSpan={7} className="p-6 text-center text-slate-400">No cities found.</td></tr>
                ) : (
                  cityList.map((city: any, index: number) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-300">{city.country_id?.country_name}</td>
                      <td className="px-4 py-3 text-slate-300">{city.region_id?.region_name}</td>
                      <td className="px-4 py-3 text-slate-300">{city.province_id?.province_name}</td>
                      <td className="px-4 py-3 font-medium text-slate-100">{city.city_name}</td>
                      <td className="px-4 py-3 text-slate-300">{city.updatedAt ? new Date(city.updatedAt).toLocaleDateString() : "-"}</td>
