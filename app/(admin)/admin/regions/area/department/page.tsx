...
import { useAddAreaDepartmentHead, useAddAreaDepartmentStaff, useGetAreaDepartmentCompleteData, useGetAreaUsers, useRemoveAreaDepartment, useRemoveAreaDepartmentHead, useRemoveAreaDepartmentStaff } from '@/query/business/queries';
...
const AreaDepartmentPage = () => {
    ...
    const { regionData, areaData, departmentData } = useSelector((state: RootState) => state.application);
    ...
    const handleFetchCompleteDepData = async () => {
        const res = await getCompleteData(departmentData?._id);
        if (res?.status === 200) {
            console.log("Res: ", res);
    ...
    const handleRemoveDepartment = async () => {
        const res = await removeAreaDepartment(departmentData?._id);
        if (res?.status === 200) {
    ...
    const handleAddDepartmentHead = async () => {
        ...
        const res = await addAreaDepartmentHead(formData);
        if (res?.status === 200) {
    ...
    const handleAddDepartmentStaff = async () => {
        ...
        const res = await addAreaDepartmentStaff(formData);
        if (res?.status === 200) {
    ...
    const handleRemoveDepartmentHead = async (headId: string) => {
        const res = await removeAreaDepartmentHead(headId);
        if (res?.status === 200) {
    ...
    const handleRemoveDepartmentStaff = async (staffId: string) => {
        const res = await removeAreaDepartmentStaff(staffId);
        if (res?.status === 200) {
    ...
    const handleViewSubDepartment = (subDepartment: any) => {
        // dispatch the subdepartment as department data and this department as the parent department.
        // route to location department page.
    }
    ...
    return (
        <div className='p-4 pb-20'>
            ...
            <div className="bg-slate-950/50 rounded-lg p-3 mb-2 min-h-[15vh]">
                <div className='flex items-center justify-between'>
                    <div>
                        ...
                        <p className='text-xs pl-1 font-medium text-slate-400 lg:w-2/3 capitalize'>Department Heads of {departmentData?.type} Department in {regionData?.region_name}, {areaData?.area_name}, those can manage the staffs, projects and connect with the co-departments in {areaData?.area_name}.</p>
                    </div>
                    <motion.div
                        whileTap={{ scale: 0.96 }}
            ...
            {/* Sub Departments ( Departments of same type in multiple location of this area ) */}
            <div className="bg-slate-950/50 rounded-lg p-3 mb-2 min-h-[15vh]">
                <div className='flex items-center justify-between'>
                    <div>
                        <h1 className='text-sm font-medium text-slate-300 capitalize flex items-center gap-1'><InfoIcon size={14} /> Sub Departments</h1>
                        <p className='text-xs pl-1 font-medium text-slate-400 lg:w-2/3 capitalize'>Sub departments of {departmentData?.type} department in {areaData?.area_name} are the {departmentData?.type} departments functioning under multiple locations of this department area.</p>
                    </div>
                    <motion.div onClick={() => handleShowMessageDialog('Adding Sub Deparments !', `Sub Departments for ${departmentData?.type} department "${departmentData?.dep_name}" in area "${areaData?.area_name}" can be only done by adding ${departmentData?.type} type departments in locations of area "${areaData?.area_name}". 🤷‍♂️`)} whileTap={{ scale: 0.96 }} className='flex items-center gap-1 cursor-pointer bg-gradient-to-tr from-slate-950/50 to-slate-950/70 rounded-lg p-2 hover:bg-slate-950/70 border border-slate-700 hover:border-cyan-600 group'>
                        <Plus size={14} className='group-hover:text-cyan-600' />
                ...
                {!loadingCompleteData && subdeps?.length === 0 && (
                    <div className="flex items-center justify-center h-[15vh]">
                        <h1 className="text-xs font-medium text-slate-300">{`No Sub Departments of ${departmentData?.type} department found on ${areaData?.area_name} locations.`}</h1>
                    </div>
                )}
                <div className="flex flex-wrap mt-1">
                                    ...
                                    <div className='flex flex-col items-start gap-1 bg-black rounded-lg'>
                                        <div className='w-full p-0.5 space-y-1'>
                                            <motion.div
                                                whileTap={{ scale: 0.98 }}
                                                whileHover={{ scale: 1.02 }}
                                                onClick={() => handleViewSubDepartment(subDepartment)}
                                                className='bg-slate-800/50 w-full p-1 py-2 text-cyan-500 cursor-pointer hover:text-cyan-700 flex items-center justify-center gap-1 border border-dashed border-slate-700 rounded-lg'>
                                                <Eye size={14} />
            ...
            <div className="bg-slate-950/50 rounded-lg p-3 mb-2 min-h-[15vh]">
                <div className='flex items-center justify-between'>
                    <div>
                        ...
                        <p className='text-xs pl-1 font-medium text-slate-400 lg:w-2/3 capitalize'>Department Staffs of {departmentData?.type} Department in {regionData?.region_name}, {areaData?.area_name}, those could complete the tasks assigned.</p>
                    </div>
                    <motion.div
                        whileTap={{ scale: 0.96 }}
                        className='flex items-center gap-1 cursor-pointer bg-gradient-to-tr from-slate-950/50 to-slate-950/70 rounded-lg p-2 hover:bg-slate-950/70 border border-slate-700 hover:border-cyan-600 group'
                ...
                {!loadingCompleteData && staffs?.length === 0 && (
                    <div className="flex items-center justify-center h-[15vh]">
            ...
            <Dialog open={addStaffOpen} onOpenChange={setAddStaffOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>Adding {isAddingStaff ? 'Staff' : 'Head'} For {departmentData?.type} Department {departmentData?.dep_name} of {regionData?.region_name}.</DialogDescription>
                    </DialogHeader>
                    <div className="">
                        {areaUsers?.length === 0 && <div className='w-full h-[10vh] flex items-center justify-center'>
                            <h1 className="text-xs font-medium text-slate-400">{`No Users with area (${areaData?.area_name}) found.`}</h1>
                        </div>}
                        {areaUsers?.map((user: any) => <motion.div
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            key={user?._id}
                        ...
                        >
                            {(addingDepartmentHead || addingDepartmentStaff) ? <LoaderSpin size={22} /> : <CircleCheckBig className="group-hover:text-cyan-600" size={18} />} {(addingDepartmentHead || addingDepartmentStaff) ? 'Adding' : 'Add'} Department {isAddingStaff ? 'Staff' : 'Head'}
                        </motion.div>
                    </div>
                </DialogContent>
            </Dialog>
