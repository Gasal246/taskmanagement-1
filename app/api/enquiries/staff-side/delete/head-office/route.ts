...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await Eq_camps.updateMany({ headoffice_id: head_office_id }, { $set: { headoffice_id: null } });
    await Eq_camp_headoffice.findByIdAndDelete(head_office_id);

    return NextResponse.json({ message: "Head office removed", status: 200 }, { status: 200 });
  } catch (err) {
    console.log("Error while deleting staff head office:", err);
