...
export async function PUT(req: NextRequest) {
  try {
    ...
    const headOffice = await Eq_camp_headoffice.findOne({
      _id: body.head_office_id,
    ...
    if (Object.keys(updatePayload).length) {
      await Eq_camp_headoffice.findByIdAndUpdate(body.head_office_id, { $set: updatePayload });
    }

    if (Array.isArray(body.camp_ids)) {
      const currentCamps = await Eq_camps.find({ headoffice_id: body.head_office_id }).select("_id").lean();
    ...
    return NextResponse.json({ message: "Head office updated", status: 200 }, { status: 200 });
  } catch (err) {
    console.log("Error while updating staff head office:", err);
