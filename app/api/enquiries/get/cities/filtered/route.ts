...
export async function GET(req: NextRequest) {
  try {
    ...
    if (search) {
      ...
      const [countries, regions, provinces] = await Promise.all([
      ...
      const orConditions: any[] = [{ city_name: regex }];
    ...
    const totalRecords = await Eq_city.countDocuments(query);
    const cities = await Eq_city.find(query)
      .populate("country_id")
      .populate("region_id")
    ...
    console.log("Error while getting cities filtered: ", err);
    return NextResponse.json(
